...
| 2. 첫 tool call까지 관측 구간 | 약 5초 | 2. 첫 tool call까지 관측 구간 | 27.1초 |
...
| 7. Cleanup 및 기타 오버헤드 | 약 32초 | 7. Cleanup 및 기타 오버헤드 | 약 14초 |
| **전체** | **약 99초** | **전체** | **약 218초** |
...
확인한 샘플 로그에서 GPT-OSS의 27.1초는 다음과 같이 분해된다.

| 구성 | Claude Audit 샘플 | GPT-OSS Audit 샘플 |
| --- | ---: | ---: |
| 초기 `uname -p` Audit | 약 7.1초 | 약 23.9초 |
| Audit 해제 후 첫 tool call까지 | 약 4초 | 약 3.2초 |

따라서 두 실험에서 사용하는 Agent 모델/API가 같다는 사실과 관측 결과는
충돌하지 않는다. 큰 차이는 Agent 추론보다는 초기 하위 프로세스를 검사한
Audit backend의 응답시간에서 발생했다.

> Claude 샘플의 위 세부 분해는 별도 샘플 로그의 초 단위 timestamp를 이용한
> 값이다. 상단의 약 99초 집계표와 동일한 실행의 정밀 API latency라고 단정할
> 수 없다. 순수 Agent latency 비교를 위해서는 Agent API 요청 직전과 응답 직후
> timestamp를 별도로 계측해야 한다.
