...
class EditTool(BaseAnthropicTool):
    ...
    async def view(self, path: Path, view_range: list[int] | None = None):
        ...
        if path.is_dir():
            ...
            try:
                ...
                raise ToolError(f"Ran into {e} while running view command")

            if not stderr and exitcode == 0:
                stdout = f"Here's the files and directories up to 2 levels deep in {path}, excluding hidden items:\n{stdout}\n"
            else:
                raise ToolError(f"The view command has exited with returncode {str(exitcode)}")
    ...
    async def _read_file(self, path: Path, timeout=120.0):
        # TODO shellescape
        process = await asyncio.create_subprocess_shell(
            f"cat {shlex.quote(str(path))}", stdin=asyncio.subprocess.PIPE, stdout=asyncio.subprocess.PIPE, stderr=asyncio.subprocess.PIPE
        )
        ...
        try:
            ...
            if stderr is not None and stderr != b"":
                raise ToolError(stderr.decode().strip())

            if process.returncode != 0:
                raise ToolError(f"file reading exited with exitcode {process.returncode}")
    ...
    async def _write_file(self, path: Path, file: str, timeout=120.0):
        # TODO shellescape
        process = await asyncio.create_subprocess_shell(
            f"cat > {shlex.quote(str(path))}", stdin=asyncio.subprocess.PIPE, stdout=asyncio.subprocess.PIPE, stderr=asyncio.subprocess.PIPE
        )
        ...
        try:
            stdout, stderr = await asyncio.wait_for(process.communicate(file.encode()), timeout=timeout)

            if stderr is not None and stderr != b"":
                raise ToolError(f"{stderr.decode().strip()}")

            if process.returncode != 0:
                raise ToolError(f"file writing exited with exitcode {process.returncode}")
    ...
    async def write_file(self, path: Path, file: str):
        ...
        try:
            await self._write_file(path, file)
        except Exception as e:
