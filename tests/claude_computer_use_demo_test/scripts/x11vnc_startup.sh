...
(x11vnc -display $DISPLAY \
    ...
    -rfbport 5900 \
    -nopw \
    2>$HOME/scripts/x11vnc_stderr.log) &
...
if [ $timeout -eq 0 ]; then
    ...
    cat $HOME/scripts/x11vnc_stderr.log >&2
    exit 1
fi

: > $HOME/scripts/x11vnc_stderr.log
...
(
    while true; do
        if ! kill -0 $x11vnc_pid 2>/dev/null; then
            ...
            if [ -f $HOME/scripts/x11vnc_stderr.log ]; then
                echo "x11vnc stderr output:" >&2
                cat $HOME/scripts/x11vnc_stderr.log >&2
                rm $HOME/scripts/x11vnc_stderr.log
            fi
            exec "$0"
        fi
        sleep 5
